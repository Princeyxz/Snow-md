...
async function connectMongo() {
    try {
        ...
        console.log('[MongoDB] ✅ Connexion réussie à', DB_NAME);
    } catch (err) {
...
async function saveSessionToMongo(num, sock) {
    if (!mongoDb) {
        console.warn('[MongoDB] ⚠️  Base non disponible, session non enregistrée.');
        return;
    }
    try {
        const collection = mongoDb.collection('sessions');
